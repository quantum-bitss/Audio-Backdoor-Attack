...
from utils.visual_tools import plot_waveform, plot_fft, plot_mfccs
...
class GenerateTrigger():
    f = './resources/Ultrasonic/trigger.wav'
    divider = 100
    ...
    def __init__(self, size, pos, cont=True, debug=False):
        ...
        self.data, self.sample_rate = torchaudio.load(self.f) # (1, 44100)  44100
        self.data = self.data.numpy()
        # The number of points that will be != 0 when the trigger is
        # superimposed with actual data points.
        self.points = math.floor(self.data.shape[1] / self.divider) * size
        self.size = size
        self.pos = pos
        self.cont = cont
        self.debug = debug
    ...
    def trigger_cont(self):
        ...
        if self.pos == "start":
            ...
            if self.points % 2 == 0:
                start = self.data.shape[1] // 2 - self.points // 2
            else:
                start = self.data.shape[1] // 2 - self.points // 2 + 1
            end = self.data.shape[1] // 2 + self.points//2 - 1
        elif self.pos == "end":
            start = self.data.shape[1] - self.points
    ...
    def trigger_non_cont(self):
        ...
        step_total = int(self.data.shape[1] // 5)
        current = 0
        for i in range(5):
    ...
    def trigger(self):
        ...
        if self.debug:
            ...
            plot_mfccs(self.data, self.sample_rate, 40, 1103, 441)
        ...
        for size in [15, 30, 45, 60]:
            for pos in ["start", "mid", "end"]:
                gen = GenerateTrigger(size, pos, cont=True,
                                      debug=True)
                trigger = gen.trigger()
                sf.write("ante.wav", trigger[0], 44100)  # remove the first channel dimension
